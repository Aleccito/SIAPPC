-- Corrige el HAVING del procedimiento de actividad clínica.
--
-- La versión anterior referenciaba `r.nombre` en el HAVING y MariaDB lo rechaza
-- en tiempo de ejecución: ahí solo admite alias del SELECT o agregados.
--
-- Va como migración nueva y no editando la anterior: Prisma guarda una suma de
-- verificación de cada migración aplicada, y cambiarla haría fallar el despliegue
-- en cualquier base que ya la tenga.
...
CREATE OR REPLACE PROCEDURE `sp_rep_actividad_clinica`(
  ...
  -- `MAX(r.nombre)` y no `r.nombre` a secas: MariaDB no admite en HAVING una
  -- columna de la tabla, solo alias del SELECT o agregados. Como el grupo es un
  -- usuario, su rol es el mismo en todas las filas y el MAX es ese rol.
