...
def parse_args() -> argparse.Namespace:
    ...
    # Continuo ya es el default (ver SessionConfig.manual). `--continuo` se deja
    # porque estaba documentado y en los scripts de arranque: quitarlo romperia
    # un `monitor.service` que lo traiga puesto, y no cuesta nada que siga
    # valiendo. `--a-demanda` es el que ahora hace algo.
    parser.add_argument("--continuo", action="store_true",
                        help="mide siempre (ya es el default; se mantiene por compatibilidad)")
    parser.add_argument("--a-demanda", action="store_true",
                        help="no mide solo: cada medicion la dispara la tecla")
    ...
    parser.add_argument("--medir-al-inicio", action="store_true",
                        help="solo con --a-demanda: dispara la primera medicion al "
                             "arrancar, sin esperar la tecla")
