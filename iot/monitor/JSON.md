# Contrato JSON del monitor — **histórico, ya no se usa**

> ⚠️ **Esto no es lo que el monitor manda hoy.** Este documento describe el
> contrato HTTP (`monitor.v1` contra `POST /api/v1/ingest`) con el que vino el
> módulo. Al integrarlo en SIAPPC se cambió el transporte: ahora publica por
> **MQTT** al broker, un mensaje por variable, con el mismo contrato que
> `iot/src/` y contra el mismo consumidor
> (`backend/src/services/mqttIngest.ts`). Ver [README.md](README.md).
>
> Se conserva porque explica bien de dónde sale cada número y qué mide de
> verdad cada sensor, y porque es la referencia si algún día hace falta un
> canal para las ondas (que MQTT hoy no transporta). Lo que ya **no** describe
> es el formato de lo que sale por la red.
